	...
	Describe("#UseFlowAnnotation", func() {
		...
		Context("control plane migration", func() {
			It("should do nothing if the shoot is in restore", func() {
				cluster.Seed.Annotations[aliapi.SeedAnnotationKeyUseFlow] = "true"
				...
				Expect(newInfra.Annotations[aliapi.AnnotationKeyUseFlow]).To(BeEmpty())
			})
		})

		Context("infrastructure creation", func() {
			It("should add global use-flow annotation if shoot contains it", func() {
				cluster.Shoot.Annotations[aliapi.GlobalAnnotationKeyUseFlow] = "foo"
				newInfra := &extensionsv1alpha1.Infrastructure{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "dummy",
						Namespace: shootNamespace,
				...
				err := mutator.Mutate(ctx, newInfra, nil)
				Expect(err).ToNot(HaveOccurred())
				Expect(newInfra.Annotations[aliapi.AnnotationKeyUseFlow]).To(Equal("foo"))
			})

			It("should add use-flow annotation if shoot contains it", func() {
				cluster.Shoot.Annotations[aliapi.AnnotationKeyUseFlow] = "foo"
				newInfra := &extensionsv1alpha1.Infrastructure{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "dummy",
						Namespace: shootNamespace,
			...
			It("should add use-flow annotation if seed label is set to new", func() {
				cluster.Seed.Annotations[aliapi.SeedAnnotationKeyUseFlow] = aliapi.SeedAnnotationUseFlowValueNew
				newInfra := &extensionsv1alpha1.Infrastructure{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "dummy",
						Namespace: shootNamespace,
